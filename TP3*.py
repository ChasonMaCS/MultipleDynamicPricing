...
np.savetxt(".simulation/3Regrets_n%d_T_%d*.txt"%(n,T_e), regretVec, fmt='%.10f')
np.savetxt(".simulation/3TotalRegrets_n%d_T_%d*.txt" % (n, T_e), totalregretVec, fmt='%.10f')

#save regret ratio to file
for t in range(T_e):
    if(totalMarketValue[t] != 0):
        regretRatio[t] = totalregretVec[t] * 1.0 / totalMarketValue[t]
np.savetxt(".simulation/3RegretRatio_n%d_T_%d*.txt" % (n, T_e), regretRatio, fmt='%.10f')