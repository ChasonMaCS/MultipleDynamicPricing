...
"""
regretVec = np.zeros(T_e, float)
totalregretVec = np.zeros(T_e, float)
...
for t in range(T_e):
    ...
    bt_T = np.dot(A, xt_T) / (1.0 * math.sqrt(np.dot(np.dot(xt, A), xt_T))) 
    bt = bt_T.transpose() 
    ...
    #posted price pt1, pt2, pt3 or pt4
    if((pt_upper - pt_lower) > epsilon):
        ...
        if pt1 <= vt:
            ...
            rho = np.sqrt(4 * (1 - alphat**2) + n**2 * alphat**4)
            delta = n**2 * (1 - (alphat**2 - rho / n) / 2) / (n**2 - 1)
            sigma = (n + 2 * (1 - rho / 2) / (alphat**2)) / (n + 1)
            psi = sigma * alphat / 2
            if pt2 <= vt:
                nextA = delta * (A - sigma * np.dot(bt_T,bt))
                nextc = c + psi*bt
                A = nextA
                c = nextc
                regret = pt1 - pt2
            else:
                pt3 = pt-alphat * (pt-pt_lower) #(1 - alphat)*pt
                #print("Posted price3: %f"%pt3)
                if pt3 <= vt:
                    nextA = delta * (A - sigma * np.dot(bt_T,bt))
                    nextc = c - psi*bt
                    A = nextA
...
np.savetxt(".simulation/3RegretsPure_n%d_T_%d.txt"%(n,T_e), regretVec, fmt='%.10f')
np.savetxt(".simulation/3TotalRegretsPure_n%d_T_%d.txt" % (n, T_e), totalregretVec, fmt='%.10f')

#save regret ratio to file
for t in range(T_e):
    if(totalMarketValue[t] != 0):
        regretRatio[t] = totalregretVec[t] * 1.0 / totalMarketValue[t]
np.savetxt(".simulation/3RegretRatio_Pure_n%d_T_%d.txt" % (n, T_e), regretRatio, fmt='%.10f')
