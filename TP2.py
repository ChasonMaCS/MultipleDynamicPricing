...
#regret vector
np.savetxt(".simulation/2RegretsPure_n%d_T_%d.txt"%(n,T_e), regretVec, fmt='%.10f')
np.savetxt(".simulation/2TotalRegretsPure_n%d_T_%d.txt" % (n, T_e), totalregretVec, fmt='%.10f')

#regret ratio
for t in range(T_e):
    if(totalMarketValue[t] != 0):
        regretRatio[t] = totalregretVec[t] * 1.0 / totalMarketValue[t]
np.savetxt(".simulation/2RegretRatio_Pure_n%d_T_%d.txt" % (n, T_e), regretRatio, fmt='%.10f')
